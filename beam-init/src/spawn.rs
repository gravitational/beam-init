...
impl MonitorCommandSender {
    ...
    pub(crate) fn signal(&mut self, sig: c_int) -> io::Result<()> {
        self.send(MonitorCommand::Signal(sig))
    }

    pub(crate) fn foreground(&mut self) -> io::Result<()> {
        self.send(MonitorCommand::Foreground)
    }

    pub(crate) fn background(&mut self) -> io::Result<()> {
        self.send(MonitorCommand::Background)
    }
}
...
impl MonitorCommand {
    fn ser(self) -> u8 {
        match self {
            MonitorCommand::Signal(sig) => {
                (sig.cast_unsigned()).try_into().expect("invalid signal")
            }
    ...
    unsafe {
        unsafe_fork!({
            ...
            if !has_ctty {
                ...
                let mut signals =
                    expect_no_panic(SignalFd::new(&sigset), "failed to create signalfd");
                ...
                loop {
                    ...
                    if fds[0].revents & POLLIN != 0 {
                        ...
                        match cmd {
                            ...
                            MonitorCommand::Foreground => {
                                ...
                                expect_no_panic(
                                    cerr(killpg(service_pid, SIGCONT)),
                    ...
                    if fds[1].revents & POLLIN != 0 {
                        expect_no_panic(
                            signals.read(),
                            "failed to read a signal from a readable signalfd",
                        );
