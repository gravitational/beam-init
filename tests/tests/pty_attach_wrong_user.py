...
result = beamctl(["testing-get-pty-from-store", "sleep"], uid=UID_B, gid=GID_B)
assert result.returncode != 0, "user B was allowed to get pty for user A's service"
assert "pty not found in store" in result.stderr, result.stderr

# User A can get the pty
result = beamctl(["testing-get-pty-from-store", "sleep"], uid=UID_A, gid=GID_A)
